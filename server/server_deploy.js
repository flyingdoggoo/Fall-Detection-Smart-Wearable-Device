...
app.post("/api/session/start", (req, res) => {
    return res.status(410).json({
        success: false,
        deploy_mode: true,
        message: "Session controls are disabled in deploy mode",
    });
});
...
app.post("/api/label", (req, res) => {
    return res.status(410).json({
        success: false,
        deploy_mode: true,
        message: "Label controls are disabled in deploy mode",
    });
});
...
app.post("/api/mark-fall", (req, res) => {
    return res.status(410).json({
        success: false,
        deploy_mode: true,
        message: "Manual markers are disabled in deploy mode",
    });
});
...
app.get("/api/status", (req, res) => {
    res.json({
        status: "running",
        mode: DEPLOY_MODE ? "deploy" : "collect",
        connectedClients,
        label: null,
        session_id: null,
        timestamp: new Date().toISOString(),
...
server.listen(PORT, "0.0.0.0", () => {
    console.log("");
    console.log("Fall detection DEPLOY server running");
...
coap.bind(COAP_PORT, "0.0.0.0", () => {
    console.log(`CoAP UDP deploy server listening on port ${COAP_PORT}`);
});
